...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False, use_cache=True):
    ...
    cache_name = args.tokenizer_name if args.tokenizer_name else list(
...
def main():
    ...
    parser.add_argument("--aggregation", default="mean", type=str, help="[mean, max, weighted, last]")

    parser.add_argument("--masking", default="full", type=str, help="[full, bidirectionnal, causal, independent]")
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model = DyReF.from_pretrained(args.output_dir, config=config, aggregation=args.aggregation,
                                      masking=args.masking,
                                      cache_dir=args.cache_dir if args.cache_dir else None)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = DyReF.from_pretrained(checkpoint, aggregation=args.aggregation, masking=args.masking,
                                          cache_dir=args.cache_dir if args.cache_dir else None)
